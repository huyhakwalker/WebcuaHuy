...
(1, '2023-01-15', 100000, 0),
(2, '2023-02-20', 75000, 1),
(3, '2023-03-10', 450000, 2),
(4, '2023-04-05', 75000, 1),
(5, '2023-05-12', 120000, 1),
(6, '2023-06-18', 50000, 1),
(7, '2023-07-22', 120000, 1),
(8, '2023-08-30', 120000, 0),
(9, '2023-09-08', 225000, 2),
(10, '2023-10-14', 30000, 1);
...
IF @MaNhanVien IS NOT NULL
       SELECT @SqlStr = @SqlStr + '
              AND (MaNhanVien LIKE ''%'+@MaNhanVien+'%'')
              '
...
IF @MaKhachHang IS NOT NULL
       SELECT @SqlStr = @SqlStr + '
              AND (MaKhachHang LIKE ''%'+@MaKhachHang+'%'')
              '
IF @TenKhachHang IS NOT NULL
       SELECT @SqlStr = @SqlStr + '
              AND (TenKhachHang LIKE ''%'+@TenKhachHang+'%'')
              '
EXEC SP_EXECUTESQL @SqlStr
End

go
create PROCEDURE MonAn_TimKiem
...
SELECT @SqlStr = '
       ...
       WHERE  (1=1)
       '
IF @MaMonAn IS NOT NULL
       SELECT @SqlStr = @SqlStr + '
              AND (MaMonAn LIKE ''%'+@MaMonAn+'%'')
              '
IF @TenMonAn IS NOT NULL
       SELECT @SqlStr = @SqlStr + '
              AND (TenMonAn LIKE ''%'+@TenMonAn+'%'')
              '
EXEC SP_EXECUTESQL @SqlStr
End
